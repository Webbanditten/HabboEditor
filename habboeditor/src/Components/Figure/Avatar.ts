...
import FigureDataPiece from "./FigureDataPiece";
...
export default class Avatar
    {
      ...
      public FiguredataReader: Array<any>;
      public Action: string; // stand
      public Gesture: string;
      public CarryItem : number;
      public Frame: number;
      public CarryDrink: number;
    
      public BodyCanvas: HTMLCanvasElement;
      public FaceCanvas: HTMLCanvasElement;
      public DrinkCanvas: HTMLCanvasElement;
    
      public CANVAS_HEIGHT = 110;
      public CANVAS_WIDTH = 64;
      public RenderEntireFigure: boolean;
      public IsOldFigure: boolean;
      public Size: string;

        public constructor(figure:string, size: string, bodyDirection: number, headDirection: number,  figuredataReader: any, action: string = "std",  gesture:string = "sml",  headOnly: boolean = false,  frame: number = 1,  carryDrink: number = -1) { 
            this.Figure = figure;
            this.Size = size.toLocaleLowerCase();
            this.IsSmall = this.Size != "b" && this.Size != "l";
            this.BodyDirection = bodyDirection;
            this.HeadDirection = headDirection;
            this.FiguredataReader = figuredataReader;
            this.RenderEntireFigure = !headOnly;
            this.IsOldFigure = figure.match(/^\d+/gm) !== null;
            
            
            if (this.IsSmall)
            {
              this.CANVAS_WIDTH = this.CANVAS_WIDTH / 2;
            ...
            if (action == "lay")
            {
                let temp = this.CANVAS_HEIGHT;
                this.CANVAS_HEIGHT = this.CANVAS_WIDTH;
                this.CANVAS_WIDTH = temp;
            ...
            this.BodyCanvas.height = this.CANVAS_HEIGHT;
            this.BodyCanvas.width = this.CANVAS_WIDTH;
            this.FaceCanvas = document.createElement("canvas");
            this.FaceCanvas.height = this.CANVAS_HEIGHT;
            this.FaceCanvas.width = this.CANVAS_WIDTH;
            this.DrinkCanvas = document.createElement("canvas");
            this.DrinkCanvas.height = this.CANVAS_HEIGHT;
            this.DrinkCanvas.width = this.CANVAS_WIDTH;
        ...
        public Run(): string[] 
        {
            var buildQueue = this.BuildDrawQueue();

            if (buildQueue == null)
                return [];

            return this.DrawImage(buildQueue);
        }

        public  TakeCareOfHats( sprite: FigureDataPiece): string
        {
            let extra = "";
        ...
        private DrawImage(buildQueue: Array<AvatarAsset> ): string[] 
        {
          buildQueue.forEach(asset => {
            console.log(asset)
          });
          return [];
          /*
...
*/
        private BuildDrawQueue(): Array<AvatarAsset> 
        {
            let tempQueue = new Array<AvatarAsset>();

            if(this.IsOldFigure /*&& this.FiguredataReader.FigurePieces.Count > 0*/)
            {
                const figureSpriteAndColor = new Array<string>(5);

                for (let i = 0; i < (this.Figure.length / 5); i++)
                {
                    const part = this.Figure.substring(i * 5, 5 * (i+1));
                    figureSpriteAndColor.push(part);
                }
                
                figureSpriteAndColor.forEach((data) => {
